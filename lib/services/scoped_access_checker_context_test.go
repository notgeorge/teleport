...
func TestScopedAccessCheckerContextAgentPin(t *testing.T) {
	...
	t.Run("Decision at pin scope allows access", func(t *testing.T) {
		...
		called := false
		err := checkerCtx.Decision(context.Background(), pinScope, func(c *ScopedAccessChecker) error {
			called = true
	...
	t.Run("Decision at child of pin scope allows access", func(t *testing.T) {
		...
		called := false
		err := checkerCtx.Decision(context.Background(), childScope, func(c *ScopedAccessChecker) error {
			called = true
			return c.CheckAccessToRules(&Context{}, types.KindNode, types.VerbRead)
		})
	...
	t.Run("Decision at root scope is denied when pin is non-root", func(t *testing.T) {
		...
		called := false
		err := checkerCtx.Decision(context.Background(), "/", func(c *ScopedAccessChecker) error {
			called = true
			return nil
		})
	...
	t.Run("RiskyAuthorizeUnpinnedRead bypasses pin enforcement", func(t *testing.T) {
		...
		err := checkerCtx.RiskyAuthorizeUnpinnedRead(context.Background(), UnpinnedReadAuthorization{
	...
	t.Run("riskyEnumerateScopedCheckers panics for agent pin context", func(t *testing.T) {
		...
		require.Panics(t, func() {
			...
			for range checkerCtx.riskyEnumerateScopedCheckers(context.Background()) {
			}
