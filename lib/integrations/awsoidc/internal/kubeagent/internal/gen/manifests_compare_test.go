...
// scenario's Options-derived values and asserts the output is equivalent
// to kubeagent.Manifests(opts). Catches drift between the chart's output
// (the source of truth) and the runtime substitutions.
...
// derived from opts and returns the decoded objects keyed by
// Kind/Name.
