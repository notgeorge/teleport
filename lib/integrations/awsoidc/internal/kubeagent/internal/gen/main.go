...
func run(arguments []string) error {
	...
	for _, v := range allVariants {
		objs, err := renderHelmChart(ch, renderOptions{valuesFile: *valuesFile, roles: v.roles(), updater: v.updater, ha: v.ha})
		if err != nil {
	...
	if err := generateSourceCode(&buf, renders); err != nil {
...
// renderHelmChart renders the helm chart and returns the decoded objects keyed by {Kind, Name}.
...
// matrix. Each variant yields its own set of variant-keyed constructors
// emitted by the generator. A public dispatcher per resource then maps
// Options to the right variant-keyed constructor.
type variant struct {
	rolesKey string
	updater  bool
	ha       bool
...
// conditionalizes on. Add rows when introducing a new configuration to
// the helm chart.
var allVariants = []variant{
	{rolesKey: "kube", updater: false, ha: false},
	{rolesKey: "kube", updater: false, ha: true},
	{rolesKey: "kube", updater: true, ha: false},
	{rolesKey: "kube", updater: true, ha: true},
	{rolesKey: "kube,app,discovery", updater: false, ha: false},
	{rolesKey: "kube,app,discovery", updater: false, ha: true},
	{rolesKey: "kube,app,discovery", updater: true, ha: false},
	{rolesKey: "kube,app,discovery", updater: true, ha: true},
}

// roles returns the comma-split roles slice for use with renderOptions.
func (v variant) roles() []string {
	return strings.Split(v.rolesKey, ",")
}

// suffix returns the Go-identifier suffix used in variant-keyed constructor
// names, e.g. "KubeUpdaterOnHaOff" or "KubeAppDiscoveryUpdaterOffHaOn".
func (v variant) suffix() string {
	...
	s.WriteString(boolSuffix("Updater", v.updater))
	s.WriteString(boolSuffix("Ha", v.ha))
	return s.String()
}

func boolSuffix(prefix string, on bool) string {
	if on {
		return prefix + "On"
	}
	return prefix + "Off"
...
// optsExpr returns the Go boolean expression that's true when an Options
// matches this variant. Emitted into the dispatcher's switch.
func (v variant) optsExpr() string {
	...
	default:
		panic(fmt.Sprintf("unknown rolesKey %q — extend optsExpr alongside allVariants", v.rolesKey))
	}
...
// based on Options.
func generateSourceCode(w io.Writer, renders map[variant]map[resourceID]runtime.Object) error {
	var body strings.Builder
	usedAliases := map[string]bool{}

	kubeOnlyTeleportConfig := extractTeleportConfig(renders[variant{rolesKey: "kube", updater: true, ha: true}])
	if kubeOnlyTeleportConfig == "" {
		return errors.New("no teleport.yaml found in ConfigMap rendered with roles=kube")
	}
	allRolesTeleportConfig := extractTeleportConfig(renders[variant{rolesKey: "kube,app,discovery", updater: true, ha: true}])
...
// emitDispatcher writes the public dispatcher for id: a function that
// switches on Options and forwards to the right variant-keyed constructor.
...
// emitConstructor writes a `func genFoo<Suffix>(opts Options) *Type { return
