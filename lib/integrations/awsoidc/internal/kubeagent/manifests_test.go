...
func TestManifests_AgentVersion(t *testing.T) {
	...
	// --base-image arg gets the agent image's repo (no tag).
...
// rendered teleport.yaml's kubernetes_service.labels (where the Teleport
// agent picks them up as static labels on the registered kube_cluster
// resource), NOT into Kubernetes pod-template labels — k8s pod labels
// have stricter validation than Teleport static labels (no colons in keys,
// values capped at 63 bytes, etc.), so AWS-style labels like
// "aws:cloudformation:stack-name" must not flow through pod metadata.
