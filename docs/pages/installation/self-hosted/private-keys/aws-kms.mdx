...
If the Auth Service is already running, execute the following command on each
Auth Service host to restart the Auth Service:

```code
$ sudo systemctl reload teleport
```

Otherwise, start Teleport on each Auth Service host:
...
on [Certificate rotation](../../../zero-trust-access/management/security/ca-rotation.mdx).
