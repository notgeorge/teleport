...
|`--mfa-mode`|`auto` (valid: `auto`, `cross-platform`, `platform`, `sso`, `browser`)|Preferred mode for MFA assertions.|
...
|`TELEPORT_MFA_MODE`|`auto` (valid: `auto`, `cross-platform`, `platform`, `sso`, `browser`)|Preferred mode for MFA assertions.|
...
|Flag|Default|Description|
|---|---|---|
|`--format`|`yaml` (valid: `yaml`, `json`, `text`)|Output format.|
...
|Flag|Default|Description|
|---|---|---|
|`--format`|`text` (valid: `yaml`, `json`, `text`)|Output format.|
...
|Flag|Default|Description|
|---|---|---|
|`--format`|`yaml` (valid: `yaml`, `json`)|Output format.|
...
|Flag|Default|Description|
|---|---|---|
|`--kind`|`user` (valid: `user`, `list`)|Access list member kind.|
...
|Flag|Default|Description|
|---|---|---|
|`--format`|`text` (valid: `json`, `text`)|Output format.|
...
|`--severity`|`low` (valid: `low`, `medium`, `high`)|Severity of the alert.|
...
|`--type`|none (valid: `host`, `db`, `db_client`, `user`)|Certificate authority type.|
...
|`--type`|none (valid: `user`, `host`, `tls-host`, `tls-user`, `tls-user-der`, `tls-spiffe`, `windows`, `db`, `db-der`, `db-client`, `db-client-der`, `openssh`, `saml-idp`, `github`, `awsra`, `app-client`)|export certificate type (user, host, tls-host, tls-user, tls-user-der, tls-spiffe, windows, db, db-der, db-client, db-client-der, openssh, saml-idp, github, awsra, app-client)|
...
|`--type`|none (valid: `host`, `windows`, `user`, `db`, `db_client`, `openssh`, `jwt`, `saml_idp`, `oidc_idp`, `spiffe`, `okta`, `awsra`, `bound_keypair`, `app_client`)|Certificate authority to rotate.|
...
|Flag|Default|Description|
|---|---|---|
|`--format`|`text` (valid: `text`, `json`)|Output format.|
|`--initial-public-key`|none|If set, use the given initial public key in SSH authorized_keys format, instead of generating a registration secret. The value must be quoted. Not compatible with --token or --legacy.|
...
|`--recovery-limit`|none|Overrides the recovery limit (default: 1) for the bound keypair token. No effect if --token or --legacy is set, or if --recovery-mode is not standard. Must be greater than 1.|
|`--recovery-mode`|`standard` (valid: `standard`, `relaxed`, `insecure`)|If set, overrides the recovery mode for the bound keypair token. No effect if --token or --legacy is set.|
|`--registration-secret`|none|Sets a registration secret for the bound keypair token. If not set, one will be randomly generated. No effect if --initial-public-key, --token, or --legacy is set. |
|`--roles`|none|Roles the bot is able to assume.|
|`--token`|none|The token to use, if any. If unset, a new single-use token will be created.|
...
|`--os`|none (valid: `linux`, `macos`, `windows`)|Operating system|
...
|`--roles-sync-mode`|none (valid: `ALL`, `NONE`)|Control account-assignment role creation. ALL creates roles for all possible account assignments. NONE creates no roles, and also implies a totally-exclusive group import filter.|
...
|`--roles-sync-mode`|`ALL` (valid: `ALL`, `NONE`)|Control account-assignment role creation. ALL creates Teleport Roles for all possible account assignments. NONE creates no Teleport Roles, and also implies a totally-exclusive group import filter.|
...
|`--user-origin`|none (valid: `defaults`, `config-file`, `dynamic`, `cloud`, `kubernetes`, `okta`, `scim`, `discovery-kubernetes`, `entra-id`, `aws-identity-center`, `integration_awsrolesanywhere`)|Shorthand for "--user-label teleport.dev/origin=ORIGIN"|
...
|Flag|Default|Description|
|---|---|---|
|`--auth`|`oauth` (valid: `bearer`, `oauth`)|Plugin Authentication type.|
|`--connector`|none|Name of the Teleport connector to use.|
|`--connector-type`|none (valid: `oidc`, `saml`)|Type of the Teleport connector to use.|
...
|`--search-mode`|`hybrid` (valid: `hybrid`, `keyword`, `embeddings`)|Search strategy to use when search queries are provided.|
...
|Flag|Default|Description|
|---|---|---|
|`-f`, `--format`|`text` (valid: `text`, `json`, `yaml`)|Format output (text, json, yaml).|
...
|`--format`|none (valid: `text`, `json`, `yaml`)|Format output (text, json, yaml).|
...
|Flag|Default|Description|
|---|---|---|
|`-f`, `--format`|none (valid: `text`, `json`, `yaml`)|Format output (text, json, yaml).|
...
|`-p`, `--preset`|none (valid: `google`, `gitlab`, `okta`)|Preset.|
...
|`--cert`|none|Cert file with the IdP certificate PEM. IdP signs \<Response\> responses using this certificate.|
|`--display`|none|Sets the connector display name.|
...
|`-p`, `--preset`|none (valid: `okta`, `onelogin`, `ad`, `adfs`)|Preset.|
...
|`--format`|none (valid: `text`, `json`, `yaml`)|Output format.|
|`--labels`|none|Set token labels, e.g. env=prod,region=us-west|
|`--ttl`|`30m0s`|Set expiration time for token, default is 30 minutes|
...
|Flag|Default|Description|
|---|---|---|
|`--bot`|none|Name of the bot that this token will grant access to. When set, creates a bot token. Overrides --type|
...
|`--type`|none (valid: `x509`)|Type of credential to revoke (x509)|
...
|`--type`|none (valid: `x509`)|Type of credential to remove the revocation for (x509).|
