...
|`--join-method`|`token` (valid: `azure`, `azure_devops`, `bitbucket`, `circleci`, `ec2`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`)|Method to use to join the cluster.|
...
|`--diag-addr`|none|Start diagnostic prometheus and healthz endpoint.|
|`-d`, `--[no-]debug`|`false`|Enable verbose logging to stderr.|
|`--gcp-alloydb-endpoint-type`|none (valid: `public`, `private`, `psc`)|(Only for AlloyDB) Endpoint type.|
...
|Argument|Default|Description|
|---|---|---|
|PROFILES|none (optional)|Comma-separated profile names to be exported. Supported profiles: goroutine,heap,mutex,trace,cmdline,profile,threadcreate,allocs,block. Default: goroutine,heap,profile|
...
|`-d`, `--[no-]debug`|`false`|Enable verbose logging to stderr.|
|`--join-method`|none (valid: `token`, `iam`, `ec2`)|Method to use to join the cluster.|
...
|`--apply-on-startup`|none|Path to a non-empty YAML file containing resources to apply on startup. Works on initialized clusters, unlike --bootstrap. Only supports the following kinds: \[bot role user token cluster_networking_config cluster_auth_preference workload_identity\].|
|`--auth-server`|none|Address of the auth server \[127.0.0.1:3025\]|
