...
|`--mfa-mode`|`auto` (valid: `auto`, `cross-platform`, `platform`, `otp`, `sso`, `browser`)|Preferred mode for MFA and Passwordless assertions.|
...
|`TELEPORT_MFA_MODE`|`auto` (valid: `auto`, `cross-platform`, `platform`, `otp`, `sso`, `browser`)|Preferred mode for MFA and Passwordless assertions.|
...
|Flag|Default|Description|
|---|---|---|
|`-f`, `--format`|none (valid: `text`, `cmd`, `json`, `yaml`)|Print format: "text" to print in table format (default), "cmd" to print connect command, "json" or "yaml" to print in JSON or YAML.|
...
|`-m`, `--mode`|`observer` (valid: `observer`, `moderator`, `peer`)|Mode of joining the session.|
...
|---|---|---|
|`-c`, `--cluster`|none|Specify the Teleport cluster to connect.|
|`-f`, `--format`|`text` (valid: `text`, `json`, `yaml`, `names`)|Format output (text, json, yaml, names).|
|`--query`|none|Query by predicate language enclosed in single quotes. Supports ==, !=, &&, and \|\| (e.g. --query='labels\["key1"\] == "value1" && labels\["key2"\] != "value2"').|
...
|`TELEPORT_MCP_CONFIG_JSON_FORMAT`|`auto` (valid: `pretty`, `compact`, `auto`, `none`)|Format the JSON file (pretty, compact, auto, none). auto saves in compact if the file is already compact, otherwise pretty. Can also be set with environment variable TELEPORT_MCP_CONFIG_JSON_FORMAT. Default is auto.|
...
|`--json-format`|`auto` (valid: `pretty`, `compact`, `auto`, `none`)|Format the JSON file (pretty, compact, auto, none). auto saves in compact if the file is already compact, otherwise pretty. Can also be set with environment variable TELEPORT_MCP_CONFIG_JSON_FORMAT. Default is auto.|
|`--labels`|none|List of comma separated labels to filter by labels (e.g. key1=value1,key2=value2).|
...
|`--type`|none (valid: `TOTP`, `WEBAUTHN`)|Type of the new MFA device (TOTP, WEBAUTHN).|
...
|---|---|---|
|`-c`, `--cluster`|none|Specify the Teleport cluster to connect.|
|`-f`, `--format`|`pty` (valid: `pty`, `json`, `yaml`, `text`)|Format output (pty, json, yaml, text).|
|`--[no-]skip-idle-time`|`false`|Quickly skip over idle time, applicable when streaming SSH or Kubernetes sessions.|
|`--speed`|`1x` (valid: `0.5x`, `1x`, `2x`, `4x`, `8x`)|Playback speed, applicable when streaming SSH or Kubernetes sessions.|
...
|---|---|---|
|`-c`, `--cluster`|none|Specify the Teleport cluster to connect.|
|`-p`, `--port`|none|Specifies the listening port used by the proxy app listener. Accepts an optional target port of a multi-port TCP app after a colon, e.g. "1234:5678".|
...
|---|---|---|
|`--app`|none|Optional Name of the AWS application to use if logged into multiple.|
|`-f`, `--format`|`unix` (valid: `unix`, `command-prompt`, `powershell`, `text`, `athena-odbc`, `athena-jdbc`)|Optional format to print the commands for setting environment variables, one of: unix, command-prompt, powershell, text. Default is unix. Or specify a service format, one of: athena-odbc, athena-jdbc.|
...
|---|---|---|
|`--app`|none|Optional Name of the Azure application to use if logged into multiple.|
|`-f`, `--format`|`unix` (valid: `unix`, `command-prompt`, `powershell`, `text`)|Optional format to print the commands for setting environment variables, one of: unix, command-prompt, powershell, text. Default is unix.|
|`-p`, `--port`|none|Specifies the source port used by the proxy listener.|
...
|---|---|---|
|`-c`, `--cluster`|none|Specify the Teleport cluster to connect.|
|`-p`, `--port`|none|Specifies the listening port used by the proxy app listener.|
...
|Flag|Default|Description|
|---|---|---|
|`-f`, `--format`|`text` (valid: `text`, `json`, `yaml`)|Format output (text, json, yaml).. Defaults to 'text'.|
...
|Flag|Default|Description|
|---|---|---|
|`--dirs`|none|Directories to scan. Defaults to /home/ on Linux, /Users/ on macOS, and C:\Users\ on Windows.|
...
|---|---|---|
|`-f`, `--format`|`text` (valid: `text`, `json`, `yaml`)|Format output (text, json, yaml).|
|`--kind`|`ssh`,`k8s`,`db`,`app`,`desktop` (valid: `ssh`, `k8s`, `kube`, `db`, `app`, `desktop`)|Filter by session kind(s).|
...
|---|---|---|
|`TELEPORT_NO_RESUME`|`false`|Disable SSH connection resumption.|
|`TELEPORT_REQUEST_MODE`|`resource` (valid: `off`, `resource`, `role`)|Type of automatic Access Request to make.|
...
|`-p`, `--port`|none|SSH port on a remote host.|
|`--reason`|none|The purpose of the session.|
|`--request-mode`|`resource` (valid: `off`, `resource`, `role`)|Type of automatic Access Request to make.|
|`--request-reason`|none|Reason for requesting access.|
