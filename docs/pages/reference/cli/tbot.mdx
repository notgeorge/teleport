...
|`--log-format`|`text` (valid: `json`, `text`)|Controls the format of output logs. Can be `json` or `text`. Defaults to `text`.|
...
|`--join-method`|none (valid: `azure`, `azure_devops`, `bitbucket`, `circleci`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`)|Method to use to join the cluster.|
...
|`--destination`|none|A destination URI, such as file:///foo/bar|
|`--diag-addr`|none|If set and the bot is in debug mode, a diagnostics service will listen on specified address.|
|`--format`|`` (valid: ``, `tls`, `mongo`, `cockroach`)|The format of the credentials to generate.|
|`--join-method`|none (valid: `azure`, `azure_devops`, `bitbucket`, `circleci`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`)|Method to use to join the cluster.|
|`--join-uri`|none|An optional URI with joining and authentication parameters. Individual flags for proxy, join method, token, etc may be used instead.|
|`--[no-]oneshot`|`false`|If set, quit after the first renewal.|
|`--[no-]scoped`|`false`|Indicates whether tbot should run in scoped mode. This is required when authenticating as a scoped Bot.|
