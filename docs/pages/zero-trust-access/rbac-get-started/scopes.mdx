...
- **Delegated administration.** Cluster administrators can grant powerful
  administrative capabilities to "scoped admins" without those admins being
...
Scoped roles serve a similar function to unscoped roles, but are not
identical. For example, label selectors are structured objects rather than
bare maps, and many unscoped role features do not yet have a scoped
equivalent.
...
to Kubernetes clusters within `/examples` (replace `ubuntu` with your
desired OS login):
...
The Teleport Terraform provider supports the `teleport_scoped_role`,
`teleport_scoped_role_assignment`, and `teleport_scoped_token` resources.
...
The Kubernetes Operator supports scoped resources via the
`TeleportScopedRoleV1`, `TeleportScopedRoleAssignmentV1`, and
`TeleportScopedTokenV1` custom resources. The Helm values used to install
the cluster must set `TELEPORT_UNSTABLE_SCOPES=yes` in the operator's
environment:
