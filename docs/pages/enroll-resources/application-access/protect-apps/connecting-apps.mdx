...
1. Create a user called `appuser` with the `demo-app-access` role:
   ...
   through the Teleport Web UI, the Teleport Proxy Service forwards the
...
An application name must be unique across root and leaf clusters if you
are deploying in a trusted clusters environment. The exact rules depend
on how the app is registered:

- **Static config (`app_service.apps` in YAML)** must use a valid DNS-1123
  label: lowercase alphanumeric or `-`, max 63 characters, no dots, must
  start and end with alphanumeric. If the configured name contains
  uppercase letters, Teleport lowercases it and logs a warning so you
  know to update your YAML; duplicate names (case-insensitive) are
  rejected at startup.
- **Dynamic resources (`tctl create app`, integrations, the Kubernetes
  operator)** accept the looser DNS-1123 subdomain form: lowercase
  alphanumeric, `-`, or `.`, max 253 characters. Mixed-case names are
  rejected with a clear error rather than silently lowercased so the
  backend storage key is not changed by a silent rename.

In both cases, `public_addr` (if set) must be a bare DNS hostname; URI
schemes, ports, and IP addresses are rejected.
...
[role templates](../../../zero-trust-access/rbac-get-started/role-templates.mdx). In the
