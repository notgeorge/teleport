...
Finally, if you are configuring the `orphaned_resource_owner` option, you must
provide the admin user with membership to this user. For example, if
`orphaned_resource_owner` is set to `bob`:
```sql
...
If you are configuring the `orphaned_resource_owner` option, you must provide the
admin user with membership to this user. For example, if `orphaned_resource_owner`
is set to `bob`:
```sql
GRANT bob TO "teleport-admin";
```
