...
When Teleport invokes the Azure run command API to install the agent, the
script output and any API errors are captured in `azure.run` audit events.
Use `tctl discovery nodes --cloud=azure` to surface this data per VM (see
below). Additional run command logs are also written on the targeted VM at
