...
Windows Desktop Service supports logins for users outside of a given `windows_desktop_service` instance's
configured domain so long as there is an outgoing trust relationship from the primary (Teleport configured)
domain to user's domain. Following the example above where Teleport is configured against the root domain
`example.com` and there exists child domain, `dev.example.com`, Teleport can authenticate the user `user@dev.example.com`
for logon to a desktop in the root domain. Similarly, `example.com` could be configured with an outgoing trust
relationship to an external domain `partner.com` to allow its users to access desktops in `example.com`.
...
#### Teleport Configuration for Cross-Domain Login

If the user belongs to a another domain within the same forest of the Teleport configured domain,
LDAPS authentication works without further configuration because user's domain is enrolled in
the same forest-wide PKI heirarchy as the root domain. This is not the case for users belonging
to a domain in another forest.

In order to authenticate users from another forest, you the following steps from the domain controller(s)
in the external forest:
- [Export the Teleport Certificate](#export-the-teleport-certificate)
- [Publish the Teleport CA to the Active Directory domain](#step-37-publish-the-teleport-ca-to-the-active-directory-domain)
- [Publish the Teleport CA to the NTAuth Store](#publish-the-teleport-ca-to-the-ntauth-store)

Additionally, repeat the [Export your LDAP CA Certificate](#step-57-export-your-LDAP-CA-certificate) step for this domain. Multiple CA certificates can be
written to the `ldap_ca_cert` configuration field for the Teleport LDAPS client to trust.
...
as a local Windows user. Additionally, NLA is not supported for cross-domain logins.
