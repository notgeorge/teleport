...
sidebar_label: Identity Activity Center
...
- A running self-hosted Access Graph node v1.28.0 or later. Read one of the
  following guides for your platform:

  - [Kubernetes (Helm chart)](self-hosted-helm.mdx)
  - [Linux server](self-hosted.mdx)
...
enabling it to access the resources and execute queries. If using an EC2 instance
...
## Step 2/3. Configure MaxMind GeoIP City database (optional)
...
## Step 3/3. Modify Identity Security configuration

Modify the Identity Activity Center configuration on your Access Graph service
and restart the service:

<Tabs>
<TabItem label="Self-hosted cluster" >

1. On the host that runs the Access Graph service, update the Identity Security
   configuration and append the following section:
...
1. Find the Docker container that runs the Access Graph service:

   ```code
   $ docker ps
   ```

1. Restart the Access Graph Service:
...
Upgrade your Identity Security Helm chart release. The command below assumes
your values file is at `tag-values.yaml`:
